...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("Steadynode"))
        return false;
...
{
    // Convert Steadynode:// to Steadynode:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Steadynode://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "Steadynode:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Steadynode.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Steadynode.lnk
...
{
    return GetAutostartDir() / "Steadynode.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a Steadynode.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Steadynode\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Steadynode app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Steadynode app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Steadynode-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Steadynode-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("Steadynode-Qt"));
