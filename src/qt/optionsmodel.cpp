...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeSteadynodeAmount"))
        settings.setValue("nAnonymizeSteadynodeAmount", 1000);
    nAnonymizeSteadynodeAmount = settings.value("nAnonymizeSteadynodeAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeSteadynodeAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeSteadynodeAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeSteadynodeAmount:
            return QVariant(nAnonymizeSteadynodeAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeSteadynodeAmount:
            nAnonymizeSteadynodeAmount = value.toInt();
            settings.setValue("nAnonymizeSteadynodeAmount", nAnonymizeSteadynodeAmount);
            emit AnonymizeSteadynodeAmountChanged(nAnonymizeSteadynodeAmount);
            break;
        default:
            break;
        }
