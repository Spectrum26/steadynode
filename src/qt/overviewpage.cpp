...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeSteadynodeAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeSteadynodeAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAmountAndRounds = strAnonymizeSteadynodeAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeSteadynodeAmount*COIN) nMaxToAnonymize = nAnonymizeSteadynodeAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeSteadynodeAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeSteadynodeAmount));
        strAmountAndRounds = strAnonymizeSteadynodeAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeSteadynodeAmount == 0){
