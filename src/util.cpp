...
int nAnonymizeSteadynodeAmount = 1000;
...
#else
    const char* pszModule = "Steadynode";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Steadynode
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Steadynode
    // Mac: ~/Library/Application Support/Steadynode
    // Unix: ~/.Steadynode
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Steadynode";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Steadynode.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "steadynoded.pid"));
