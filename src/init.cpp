...
void Shutdown()
{
    ...
    RenameThread("Steadynode-shutoff");
...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Steadynode.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: steadynoded.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizesteadynodeamount=<n> " + _("Keep N Steadynode anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Steadynode is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Steadynode is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Steadynode version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Steadynode server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Steadynode") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Steadynode to complete") << "\n";
    ...
    nAnonymizeSteadynodeAmount = GetArg("-anonymizesteadynodeamount", 0);
    if(nAnonymizeSteadynodeAmount > 999999) nAnonymizeSteadynodeAmount = 999999;
    if(nAnonymizeSteadynodeAmount < 2) nAnonymizeSteadynodeAmount = 2;
    ...
    LogPrintf("Anonymize Steadynode Amount %d\n", nAnonymizeSteadynodeAmount);
